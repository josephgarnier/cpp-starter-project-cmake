...
.. parsed-literal::
    ...
    file_manip(`ABSOLUTE_PATH`_ <input-list-var> BASE_DIR <path> [OUTPUT_VARIABLE <output-var>])
    file_manip(`GET_COMPONENT`_ <input-list> MODE <mode> OUTPUT_VARIABLE <output-var>)
...
.. code-block:: cmake
  ...
  file_manip(ABSOLUTE_PATH <input-list-var> BASE_DIR <directory-path> [OUTPUT_VARIABLE <output-var>])

Compute the absolute path from a ``<directory-path>`` to a the list of input path ``<input-list-var>`` and store the result in-place or in the specified ``<output-var>``.

.. _GET_COMPONENT:
.. code-block:: cmake

  file_manip(GET_COMPONENT <input-list> MODE <mode> OUTPUT_VARIABLE <output-var>)

Sets in the specified ``<output-var>`` to a component of file of ``<input-list-var>``, where ``<mode>`` is one of:
