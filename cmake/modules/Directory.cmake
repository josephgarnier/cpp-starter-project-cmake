...
.. parsed-literal::
    ...
    directory(`SCAN`_ <output-var> ROOT_PATH <path> INCLUDE_REGEX <regular-expression>)
...
.. code-block:: cmake
  ...
  directory(SCAN <output-var> ROOT_PATH <path> INCLUDE_REGEX <regular-expression>)
...
function(directory)
	set(options "")
	set(one_value_args SCAN ROOT_PATH INCLUDE_REGEX)
	...
	if(DEFINED DIR_SCAN)
		...
		file(GLOB relative_path RELATIVE ${CMAKE_CURRENT_SOURCE_DIR} "${DIR_ROOT_PATH}")
		file(GLOB children RELATIVE ${DIR_ROOT_PATH} "${DIR_ROOT_PATH}/*")
		set(list_dirs "")
		set(list_files "")

		foreach(child IN ITEMS ${children})
			if(IS_DIRECTORY "${DIR_ROOT_PATH}/${child}")
				list(APPEND list_dirs ${child})
			elseif("${child}" MATCHES "${DIR_INCLUDE_REGEX}")
				list(APPEND list_files "${DIR_ROOT_PATH}/${child}")
			endif()
		endforeach()
		...
		foreach(subdirectory IN ITEMS ${list_dirs})
			directory(SCAN ${DIR_SCAN} ROOT_PATH "${DIR_ROOT_PATH}/${subdirectory}" INCLUDE_REGEX "${DIR_INCLUDE_REGEX}")
		endforeach()
